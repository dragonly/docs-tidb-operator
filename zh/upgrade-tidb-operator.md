...
2. 获取你要安装的 `tidb-operator` chart 中的 `values.yaml` 文件：
    ...
    helm show values pingcap/tidb-operator --version=${version} > ${HOME}/tidb-operator/${version}/values-tidb-operator.yaml
    ```
    
3. 将旧版本 `values.yaml` 中的自定义配置合并到 `${HOME}/tidb-operator/${version}/values-tidb-operator.yaml`，然后执行 `helm upgrade`：

    {{< copyable "shell-regular" >}}

    ```shell
    helm upgrade tidb-operator pingcap/tidb-operator --version=${version} -n tidb-admin -f ${HOME}/tidb-operator/${version}/values-tidb-operator.yaml
    ```
