...
+ `.spec.pause`：该值默认为 `false`。如果将该值设置为 `true`，表示暂停定时调度。此时即使到了调度时间点，也不会进行备份。在定时备份暂停期间，备份 [Garbage Collection](https://pingcap.com/docs-cn/stable/garbage-collection-overview/) (GC) 仍然正常进行。将 `true` 改为 `false` 则重新开启定时全量备份。
